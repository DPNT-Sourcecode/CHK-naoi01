...
def get_group_discount(skus: list[SKU]) -> int:
    highest_discount_items = get_group_discount_items(skus)
    return sum(item.price for item in highest_discount_items) - 45
...
def choose_best_offer_to_apply(current_basket: list[SKU]) -> SpecialOffer:
    ...
    offer_with_largest_discount = sorted(
        applicable_offers,
        key=lambda offer: get_offer_discount(offer, current_basket),
        reverse=True,
    )[0]
